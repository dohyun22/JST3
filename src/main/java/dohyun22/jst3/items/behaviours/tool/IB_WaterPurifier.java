...
public class IB_WaterPurifier extends ItemBehaviour {
	...
	public void update(ItemStack st, World w, Entity e, int sl, boolean select) {
		if (!w.isRemote && w.getTotalWorldTime() % 100L == 0L && e instanceof EntityPlayer && getEnergy(st) >= 100) {
				...
				try {
					...
					if (ct < 20) {
						...
						if (fs.amount >= r * 30) {
							...
							FluidUtil.getFluidHandler(st).drain(Math.max(0, r * 100), true);
						}
					}
				} catch (Throwable t) {}
			}
	...
	public void getSubItems(ItemStack st, List<ItemStack> sub) {
		...
		if (fh != null) fh.fill(new FluidStack(FluidRegistry.WATER, 16000), true);
		sub.add(st);
	}
