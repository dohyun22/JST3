...
public class IB_FluidCan extends ItemBehaviour {
	...
	public ActionResult<ItemStack> onRightClick(ItemStack st, World w, EntityPlayer pl, EnumHand h) {
		...
		if (type == CanType.NONE) {
			RayTraceResult rtr = JSTUtils.rayTracePlayer(w, pl, true);
				...
				if (w.canMineBlockBody(pl, p) && pl.canPlayerEdit(p, rtr.sideHit, pl.getHeldItem(h))) {
					...
					if (far.success) {
						if (st.getCount() == 1) {
							...
							st.shrink(1);
							pl.inventory.addItemStackToInventory(far.result);
						}
	...
	private static class CanFluidHandler implements IFluidHandlerItem {
		...
		public FluidStack drain(FluidStack r, boolean d) {
			if (r == null || !r.isFluidEqual(getFluid()))
				return null;
		...
		public FluidStack getFluid() {
			if (type.fname == null)
				return null;
			return FluidRegistry.getFluidStack(type.fname, type.is144mb ? 144 : 1000);
		}
		...
		public IFluidTankProperties[] getTankProperties() {
			return new IFluidTankProperties[] {new FluidTankProperties(getFluid(), type.is144mb ? 144 : 1000)};
		}
	...
	public static enum CanType {
		...
		AIR("air"), ACID("acid"), HG("mercury"), UU("ic2uu_matter"), DSTW("ic2distilled_water"), CLNT("ic2coolant"),
		HCLT("ic2hot_coolant"), BMAS("biomass"), BETH("bio.ethanol"), CREO("creosote"), ETHA("ethanol"),
		BDSL("biodiesel"), POIL("plantoil"), SOIL("seed.oil"), DESL("diesel"), GASL("gasoline"), LUBR("lubricant"), HVYF("heavyfuel");
