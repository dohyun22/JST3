...
public class GUIFusion extends GUIBase {
	...
	protected void drawGuiContainerForegroundLayer(int p1, int p2){
		...
		if (con.complete) {
			this.fontRenderer.drawString((con.rfDisp ? (con.energy * JSTCfg.RFPerEU) + " RF" : con.energy + " EU") + " /", 12, 60, 0x00FFFF);
			this.fontRenderer.drawString(con.rfDisp ? (con.mxenergy * JSTCfg.RFPerEU) + " RF" : con.mxenergy + " EU", 12, 70, 0x00FFFF);
		} else {
			this.fontRenderer.drawString(I18n.format("jst.msg.multi.invalid"), 14, 58, 0xFF0000);
		}
	}
    ...
    public void drawScreen(int mX, int mY, float pt) {
		...
		if (!Loader.isModLoaded("jei") && isPointInRegion(152, 8, 16, 16, mX, mY))
			this.drawHoveringText(I18n.format("jst.compat.jei.missing"), mX, mY);
