...
public class ContainerFluidGen extends ContainerMTE {
	...
	public ContainerFluidGen(IInventory inv, TileEntityMeta te) {
		...
		addSlotToContainer(new InternalSlot(te, 0, 107, 22, te.mte instanceof MetaTileFluidGen ? ((MetaTileFluidGen) te.mte).type : -1));
	...
	private class InternalSlot extends Slot {
	    ...
	    public boolean isItemValid(ItemStack st) {
	        ...
	        return MetaTileFluidGen.getFuelValue(mode, FluidUtil.getFluidContained(st)) > 0;
	    }
	}
}
